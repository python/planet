...
class relativize:
    ...
    def collect_and_tally(self, parent):
        uri = None
        if parent.hasAttribute("cite"):
            uri = parent.getAttribute("cite")
        if parent.hasAttribute("href"):
            uri = parent.getAttribute("href")
        if parent.hasAttribute("src"):
            uri = parent.getAttribute("src")
        ...
        if uri:
            ...
            if parts[0].lower() == "http":
                ...
                if base and base not in self.links:
    ...
    def rebase(self, parent):
        ...
        if uri and uri.startswith(self.base):
            ...
            if parent.hasAttribute("href"):
                uri = parent.setAttribute("href", uri)
            if parent.hasAttribute("src"):
                uri = parent.setAttribute("src", uri)
