...
__author__ = ["Mark Pilgrim <http://diveintomark.org/>", "Aaron Swartz <http://www.aaronsw.com/>"]
...
def characters(text, isXML=False, guess=None):
    ...
    return (
        tryEncoding(guess)
        or tryEncoding(_detectbom(text))
        or isXML
        and tryEncoding(_detectbom(text, xml_bom_map))
        or tryEncoding(_chardet(text))
        or tryEncoding("utf8")
        or tryEncoding("windows-1252")
        or tryEncoding("iso-8859-1")
    )
